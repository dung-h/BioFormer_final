#!/usr/bin/env python3
"""
Preprocess Lung-Kim ENSEMBL data for BioFormer integration.
"""
...
def select_hvg_from_lung_kim_data(lung_kim_files, n_top_genes=1000):
    """Select highly variable genes from Lung-Kim data."""
    import scanpy as sc
    from collections import Counter
    
    # Get common genes across all files
    var_names_list = []
    for fp in lung_kim_files:
        adata = sc.read_h5ad(fp)
        var_names_list.append(set(adata.var_names))
    
    # Find common genes
    common_genes = set.intersection(*var_names_list)
    print(f"Found {len(common_genes)} common genes across Lung-Kim datasets")
...
def main():
    """Preprocess Lung-Kim data with correct cell type encoding."""
    # Define paths
    lung_kim_files = [
        'data/lung_kim_ensembl/lung_train_ensembl.h5ad',
        'data/lung_kim_ensembl/lung_test_ensembl.h5ad'
    ]
    
    output_dir = 'data/preprocessed_lung_kim'
    os.makedirs(output_dir, exist_ok=True)
    
    # Select genes from Lung-Kim data itself
    selected_genes = select_hvg_from_lung_kim_data(lung_kim_files, n_top_genes=1000)
    ...
    # Process each file
    for study_id, fp in enumerate(lung_kim_files):
