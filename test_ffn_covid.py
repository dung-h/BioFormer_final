#!/usr/bin/env python3
"""
FFN BioFormer COVID-19 Batch Integration Test
"""
import os
import sys
import h5py
...
def run_ffn_covid_test():
    """Run COVID-19 batch integration test using FFN BioFormer model."""
    ...
    print(f"\n{'='*60}")
    print(f"Running FFN BioFormer COVID-19 Integration Test: {data_dir}")
    ...
    # Load FFN model (adjusted for 454 genes instead of 1000)
    print("Loading FFN BioFormer model...")
    model = scGPT(vocab_size=454, num_cell_types=dataset.num_cell_types)  # 454 overlapping genes
    model.to(device)
    ...
    print(f"\nFFN BioFormer COVID-19 Results:")
    print(f"  NMI = {nmi:.4f}")
    print(f"  ARI = {ari:.4f}")
...
if __name__ == "__main__":
    results = run_ffn_covid_test()
    print("\nFFN COVID-19 Integration Test Completed!")